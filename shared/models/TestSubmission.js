...
const answerSchemas = {
  ...
  sectionAnswers: new mongoose.Schema({
    sectionId: {
      type: String, // Use string ID for compatibility with Test model
      required: true
    },
    answers: {
      type: Map,
...
const testSubmissionSchema = new mongoose.Schema({
  ...
  results: {
    ...
    questionAnalysis: [{
      questionId: String, // Use string for compatibility
...
testSubmissionSchema.methods.calculateScores = async function() {
  try {
    ...
    if (!this.testId.readingSections && !this.testId.reading) {
    ...
    // Get sections from either new or old structure
    const readingSections = test.reading?.sections || test.readingSections || [];
    const listeningSections = test.listening?.sections || test.listeningSections || [];
    
    // Calculate reading score
    if (readingSections.length > 0 && this.answers.reading) {
      results.reading = this.calculateReadingScore(readingSections, this.answers.reading);
    }
    
    // Calculate listening score
    if (listeningSections.length > 0 && this.answers.listening) {
      results.listening = this.calculateListeningScore(listeningSections, this.answers.listening);
    }
...
testSubmissionSchema.methods.calculateReadingScore = function(sections, answers) {
  ...
  sections.forEach((section, sectionIndex) => {
    ...
    section.questions.forEach((question, questionIndex) => {
      totalQuestions++;
      const userAnswer = sectionAnswers.answers.get(question.id || question._id?.toString());
